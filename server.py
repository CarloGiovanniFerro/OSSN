#!/usr/bin/env python3
"""
OSSN Italia v8 — Backend AIS (sorgente: aisstream.io)
...
def ais_loop():
    ...
    while True:
        ...
        try:
            ...
            raw = socket.create_connection((AIS_HOST, 443), timeout=20)
            ...
            # IMPORTANTE: rimuove il timeout dopo l'handshake.
            # Il timeout di connessione (20 s) restava attivo anche durante
            # la ricezione dati, causando "read operation timed out" ad ogni
            # pausa nel flusso AIS. Con None il recv è bloccante senza limite.
        ...
        print(f'[ais] Riconnessione in {delay}s...')
        time.sleep(delay)
...
╠══════════════════════════════════════════════════════╣
║  Sorgente: aisstream.io (real-time globale)          ║
║  Attiva "Navi AIS live" nel pannello Layer           ║
╚══════════════════════════════════════════════════════╝
""")
